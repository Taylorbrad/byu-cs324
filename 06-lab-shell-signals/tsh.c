	...
	if (argv[1] == NULL) {
		...
		if (argv[1][0] == '%') { // is job id
			...
			if (getjobpid(jobs, atoi(argv[1])) != NULL) {
				...
				// jid = getjobpid(jobs, atoi(argv[1]))->jid;
				// pid = getjobpid(jobs, atoi(argv[1]))->pid;
				jid = job->jid;
				pid = job->pid;
				// printf("update state pid\n");
			}
			if (jid == -1) {
		...
		if (!strcmp(argv[0],"bg")) {
			printf("[%d] (%i) %s", job->jid, job->pid, job->cmdline);
			// printf("[%d] (%d) ", jobs[jid-1].jid, jobs[jid-1].pid);
			jobs[jid-1].state = BG;
			kill(-pid, SIGCONT);
		}
		else {
...
void sigint_handler(int sig)
{
	// printf("sigint kill %d", sig);
	if (kill(-fgpid(jobs), sig) < 0)
		fprintf(stderr, "kill (sigint) error");
